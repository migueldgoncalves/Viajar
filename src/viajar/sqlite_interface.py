...
PASTA_CSV = 'D:\\PycharmProjects\\Viajar\\src\\viajar\\base_dados'  # A pasta do projecto Python onde estão os ficheiros .csv
FICHEIRO_BD = 'Travel'  # Sem extensão - Nome do ficheiro de BD a ser criado

PATH_BD = os.path.join(PASTA_DESTINO, FICHEIRO_BD)

SCRIPT_SQLITE = 'base_dados_sqlite.sql'
...
class SQLiteBDInterface:
    ...
    def preencher_base_dados(self):
        ...
        preencher_tabela('local.csv', "INSERT INTO Location(name, latitude, longitude, altitude, extra_info, batch) VALUES(?, ?, ?, ?, ?, ?);")
        preencher_tabela('concelho.csv', "INSERT INTO Concelho(concelho, intermunicipal_entity, district, region) VALUES(?, ?, ?, ?);")
        preencher_tabela('provincia.csv', "INSERT INTO Province(province, autonomous_community) VALUES(?, ?);")
        preencher_tabela('municipio.csv', "INSERT INTO Municipio(municipio, province) VALUES(?, ?);")
        preencher_tabela('local_portugal.csv', "INSERT INTO LocationPortugal(name, parish, concelho) VALUES(?, ?, ?);")
        preencher_tabela('local_espanha.csv', "INSERT INTO LocationSpain(name, municipio, province, district) VALUES(?, ?, ?, ?);")
        preencher_tabela('local_gibraltar.csv', "INSERT INTO LocationGibraltar(name, major_residential_area) VALUES(?, ?);")
        preencher_tabela('comarca.csv', "INSERT INTO Comarca(municipio, comarca, province) VALUES(?, ?, ?);")
        preencher_tabela('ligacao.csv', "INSERT INTO Connection(location_a, location_b, means_transport, distance, extra_info, cardinal_point, order_a, order_b) VALUES(?, ?, ?, ?, ?, ?, ?, ?);")
        preencher_tabela('destino.csv', "INSERT INTO Destination(location_a, location_b, means_transport, starting_point, destination) VALUES(?, ?, ?, ?, ?)")
